...
/**
 * === orders.php ===
 * 쿠팡 → 영카트 주문 동기화 크론 스크립트
 * 경로: /plugin/gnuwiz_coupang/cron/orders.php
 * 용도: 쿠팡에서 신규/수정된 주문을 영카트로 동기화
 * 실행주기: 매분 실행 권장 (*/1 * * * *)
 */
...
try {
    ...
    echo "[" . date('Y-m-d H:i:s') . "] 쿠팡 주문 동기화 시작\n";
    
    // 주문 동기화 실행 (지난 1시간)
    $sync_result = $coupang_api->syncOrdersFromCoupang(1);
    
    if ($sync_result['success']) {
        $message = "주문 동기화 완료 - 처리: {$sync_result['processed']}건, " .
                  "신규: {$sync_result['new_orders']}건, " . 
                  "수정: {$sync_result['updated_orders']}건, " .
        ...
        // 크론 실행 로그 기록
        monitor_cron_execution('orders', 'SUCCESS', $message, $sync_result['execution_time']);
        
        // 성공 통계 업데이트
        update_sync_statistics('orders', $sync_result['processed'], true);
    ...
    $error_message = '주문 동기화 실패: ' . $e->getMessage();
    ...
    // 크론 실행 로그 기록
    monitor_cron_execution('orders', 'FAIL', $error_message, $execution_time);
    
    // 실패 통계 업데이트
    update_sync_statistics('orders', 0, false);
...
echo "[" . date('Y-m-d H:i:s') . "] 주문 동기화 완료 - 총 실행시간: " . round($total_execution_time, 2) . "초\n";

coupang_log('INFO', '주문 동기화 크론 완료', array(
